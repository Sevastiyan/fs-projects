...
def load_combined_file(file, filter=False, regressions=None, cutoff=1.8):
    ...
    if filter:
        ...
        for col in left_data.columns:
            if col in [
                ...
                continue
            xLeft[col] = butter_lowpass_filter(left_data[col], cutoff=cutoff, fs=20)
            xRight[col] = butter_lowpass_filter(right_data[col], cutoff=cutoff, fs=20)
