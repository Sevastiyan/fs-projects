...
def load_combined_file(file, filter=False, regressions=None, frequency=20, cutoff=1.8, order=4):
    ...
    if filter:
        ...
        for col in left_data.columns:
            ...
            xRight[col] = butter_lowpass_filter(right_data[col], cutoff=cutoff, fs=frequency)
...
def convert_signal(data, type):
    ...
    if type == "acc_total":
        ...
        x = 9.81 * (np.sqrt((x["acc_x"] / 4096) ** 2 + (x["acc_y"] / 4096) ** 2 + (x["acc_z"] / 4096) ** 2) - 1)
        return x
...
def filter_response(data, cutoff=10, fs=100, order=3):
    nyq = 0.5 * fs  # Nyquist frequency
    normal_cutoff = cutoff / nyq  # Normalize the cutoff
    b, a = butter(order, normal_cutoff, btype="low", analog=False)  # Filter coefficients
    ...
    t = np.linspace(0, 0.1 * len(data), len(data), endpoint=False)
